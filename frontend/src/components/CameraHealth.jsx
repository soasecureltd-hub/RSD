...
export default function CameraHealth() {
  ...
  const [riskEstimation, setRiskEstimation] = useState(null);
  const [loading, setLoading] = useState(false);
  const [cameraId] = useState('CAM-WEBCAM-001');
  const [lastCaptureTime, setLastCaptureTime] = useState(null);
  const intervalRef = useRef(null);
  const isAnalyzingRef = useRef(false); // guard against overlapping requests
  ...
  const captureAndAnalyze = async () => {
    ...
    try {
      ...
      // Fetch Risk Estimation
      try {
        const estResponse = await cameraAPI.getCameraRiskEstimation(cameraId);
        if (estResponse.data?.status === 'success') {
           setRiskEstimation(estResponse.data.data);
        }
      } catch(err) {
        console.error('Error fetching risk estimation:', err);
      }
    ...
    <div className="camera-container">
      ...
      {health && (
        <>
          ...
          {riskEstimation && (
            <div className="history-section risk-estimation-section">
              <h3>🎯 Live Risk Estimation</h3>
              <p className="subtitle">Real-time parameters derived directly from camera data</p>
              
              <div className="metrics-grid" style={{ marginTop: '1rem' }}>
                <div className="metric-card">
                  <h4>CCTV Functionality</h4>
                  <p className="value">{riskEstimation.physical_security.cctv_functionality.toFixed(0)}%</p>
                </div>
                <div className="metric-card">
                  <h4>CCTV Coverage</h4>
                  <p className="value">{riskEstimation.physical_security.cctv_coverage}%</p>
                </div>
                <div className="metric-card">
                  <h4>Lighting Quality</h4>
                  <p className="value">{riskEstimation.physical_security.lighting_quality}</p>
                </div>
                <div className="metric-card">
                  <h4>Entry/Exit Control</h4>
                  <p className="value">{riskEstimation.physical_security.entry_exit_control}</p>
                </div>
                <div className="metric-card">
                  <h4>After Hours Security</h4>
                  <p className="value">{riskEstimation.access_control.after_hours_security}</p>
                </div>
                <div className="metric-card">
                  <h4>Shift Coverage</h4>
                  <p className="value">{riskEstimation.personnel.shift_coverage}</p>
                </div>
                <div className="metric-card">
                  <h4>Guard Count Ratio</h4>
                  <p className="value">{riskEstimation.personnel.guard_count_ratio.toFixed(0)}</p>
                </div>
                <div className="metric-card">
                  <h4>Incident Severity</h4>
                  <p className="value">{riskEstimation.incident_history.incident_severity_score.toFixed(0)}</p>
                </div>
              </div>
            </div>
          )}
